	...
	for (int k = bN ; k >= b0 ; k--){
		if (Gnr[k]!=generation) {
			...
			if (generation==0) {
				if (ring) {
					if (k==bN) {
						Lat[0]->Initiate(Gg_b + (s%2)*M*size,G_start,M); 
						Lat[0]->AddPhiS(rho+molmon_nr[k]*M, Gg_f+s*M*size, Gg_b+(s%2)*M*size, M);
						s--;
					} else {
						if (k==bN-1) {
							N= n_mon[k];
	...
	for (int k = bN ; k >= b0 ; k--){
		if (Gnr[k]!=generation) {
			...
			if (generation==0) {
				if (ring) {
					if (k==bN) {
						Lat[0]->Initiate(Gg_b + (s%2)*M,G_start,M); 
						Lat[0]->AddPhiS(rho+molmon_nr[k]*M, Gg_f+s*M, Gg_b+(s%2)*M, M);
						s--;
					} else {
						if (k==bN-1) {
							N= n_mon[k];
			...
			case 1:
				for (x=1; x<MX+1; x++) {
					...
					if (Seg[mon_nr[0]]->G1[i]>0 && doit) {
						...
						GN+=Lat[0]->ComputeGN(G,M); 
						s--;
						if (save_memory) {
							Lat[0]->Initiate(Gg_b,G0,M);
							Lat[0]->Initiate(Gg_b+M*size,G0,M); 
