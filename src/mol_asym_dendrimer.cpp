			...
			for (int b=bN; b>=b0; b--) {
				...
				for (int k=0; k<N; k++) {
					if (g==n_g-1 && s==last_s[a]) {
						Lat[0]->Initiate(Gg_f+s*M*size,Seg[mon_nr[b]]->G1,M);
					} else {
						if (b==bN &&k==0) {
							Lat[0]->Terminate(GS,Gg_f+slast*M*size,M);
...
if (debug) cout <<"Backward2ndO for mol_asym_dend " + name << endl;
	...
	if (g==0) {
		...
		Lat[0]->Terminate(GS,Gg_b+((s-1)%2)*M*size,M);
	...
	Lat[0]->AddPhiS(rho+molmon_nr[first_b[first_a[g]]-1]*M,Gg_f+s*M*size,Gg_b+(s%2)*M*size,n_repeats,M);

	for (int k=0; k<size; k++) Div(GX+k*M,Seg[mon_nr[first_b[first_a[g]]-1]]->G1,M);

	for (int a=first_a[g]; a<=last_a[g]; a++) {
		s=first_s[a];
		Lat[0]->Terminate(GS,Gg_f+s*M*size,M);
		Lat[0]->propagate(GS,UNITY,0,1,M);
		Lat[0]->Terminate(GS,GX,M);
		Div(GS,GS+M,M);
		...
		for (int b=b0; b<=bN; b++) {
			...
			for (int k=0; k<N; k++) {
				...
				Lat[0]->AddPhiS(rho+molmon_nr[b]*M,Gg_f+s*M*size,Gg_b+(s%2)*M*size,n_arm[a]*n_repeats,M);
				s++;
			}
		}
...
if (debug) cout <<"Forward for mol_asym_dend " + name << endl;
	...
	for (int g=n_g-1; g>=0; g--) {
		...
		for (int a=last_a[g]; a>=first_a[g]; a--) {
			...
			Lat[0]->Terminate(GS,Gg_f+(s+1)*M,M);
			Lat[0]->propagate(GS,UNITY,0,1,M);
			for (int k=0; k<n_arm[a]; k++) Times(GS+2*M,GS+2*M,GS+M,M);
		}
		Times(GS+2*M,GS+2*M,Seg[mon_nr[first_b[first_a[g]]-1]]->G1,M);
		Lat[0]->Initiate(Gg_f+s*M,GS+2*M,M);
		slast = s; 
	}
...
if (debug) cout <<"Backward for mol_asym_dend " + name << endl;
	...
	Lat[0]->AddPhiS(rho+molmon_nr[first_b[first_a[g]]-1]*M,Gg_f+s*M,Gg_b+(s%2)*M,n_repeats,M);
	...
	for (int a=first_a[g]; a<=last_a[g]; a++) {
		...
		for (int b=b0; b<=bN; b++) {
			...
			for (int k=0; k<N; k++) {
				...
				Lat[0]->AddPhiS(rho+molmon_nr[b]*M,Gg_f+s*M,Gg_b+(s%2)*M,n_arm[a]*n_repeats,M);
				s++;
			}
		}
...
bool mol_asym_dend::ComputePhi() {
	...
	bool success=true;
	if (Markov ==2) {
		GN=Lat[0]->ComputeGN(Forward2ndO(),M);		
		s=0; Backward2ndO(generation,1,s);
	} else {
		GN=Lat[0]->ComputeGN(Forward(),M);		
