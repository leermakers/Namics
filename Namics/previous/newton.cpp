...
if(debug) cout <<"SuperIteration in  Newton " << endl;
	...
	if (search>-1) x[0] =Var[search]->GetValue(); else {if (ets>-1) x[0] = Var[ets]->GetValue(); else x[0]=Var[etm]->GetValue();}
	...
	int k_diis=1; 
	int k=0;
	if ((ets==-1 && etm==-1)||  search<0) Solve(false); else SuperIterate(search,target,-1,-1);
	if (target>-1) g[0]=Var[target]->GetError(); else {if (ets>-1) g[0]=Var[ets]->GetError(); else g[0]=Var[etm]->GetError();}
	...
	while (residual > tol && it < iterationlimit) {
		it++;
		Cp(x0,x,iv);
		//if (ets==-1) Var[search]->PutValue(x[0]); else {if (ets>-1) Var[ets]->PutValue(x[0]); else Var[etm]->PutValue(x[0]);}
		if (search>-1) Var[search]->PutValue(x[0]); else {if (ets>-1) Var[ets]->PutValue(x[0]); else Var[etm]->PutValue(x[0]);}
		//if (ets==-1||search<0) Solve(false); else SuperIterate(search,target,-1,-1);
		if ((ets==-1 && etm==-1) ||search<0) Solve(false); else SuperIterate(search,target,-1,-1);
		//if (ets==-1) g[0]=Var[target]->GetError(); else {if (ets>-1) g[0]=Var[ets]->GetError(); else g[0]=Var[etm]->GetError();}
		if (target>-1) g[0]=Var[target]->GetError(); else {if (ets>-1) g[0]=Var[ets]->GetError(); else g[0]=Var[etm]->GetError();}
cout <<"theta " << x[0] << endl; 
		k=it % m; k_diis++; //plek voor laatste opslag
		YplusisCtimesX(x,g,-delta_max,iv);
