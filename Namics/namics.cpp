...
int main(int argc, char *argv[]) {
 ...
 //TODO: I think these two are no longer used, but I'm too scared to delete them now - Daniel
	string line;
	ofstream out_file;
...
/******** Class creation starts here ********/
  ...
  //Create variate class instance and check inputs (reference above)
	...
	//Error code for faulty variate class creation
		...
		if (n_ets>1) {cout <<"too many equate_to_solvent's in var statements. The liZZmit is 1 " << endl; return 0;}
  ...
  //Create engine class instance and check inputs (reference above)
		...
		if (!Eng[0]->CheckInput(start)) {return 0;}
  ...
  //Prepare, catch errors for output class creation
	...
	//Create output class instance and check inputs (reference above)
		...
		//TODO: 1. 	I think sys.initial_guess is always set to "previous_result" by default at system.cpp:301
		//					which would make the following obsolete.
		//			2.	What does this block do, exactly? It seems to handle some code for charged molecules.
...
/********** Generate output from every class *********/
			Lat[0]->PushOutput();
			New[0]->PushOutput();
			Eng[0]->PushOutput();
