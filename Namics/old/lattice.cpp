...
if (debug) cout <<"CheckInput in lattice " << endl; 
	...
	if (success){
		...
		FJC=3;	fjc=1;
		if (success && GetValue("FJCchoices").length()>0) {
			if (!In[0]->Get_int(GetValue("FJCchoices"),FJC,"FJCchoices can adopt only few integer values: 3 + i*4, with i = 1, 2, 3, 4, ...")) success=false;
			else {
				if (FJC %4 != 3) {cout << "FJCchoices can adopt only few integer values: 3 + i*4, with i = 1, 2, 3, 4, ...." <<endl; success=false;}
			}
			if (success && (gradients>1)) {cout << "FJCchoices can only be used in combination with gradients == 1 " << endl; success=false;}
			if (success && (lattice_type != "hexagonal")) {cout << "FJC choices can only be used in combination with lattice_type 'hexagonal' "<<endl; success=false;} 
			cout <<FJC <<"FJCchoices" << endl; 
			fjc=(FJC-3)/2+1; 
...
void Lattice::Edis(Real *X_side, Real *X, int M) { //operation opposite to Side ;-)
if (debug) cout <<"Edis in lattice " << endl; 
	...
	if (fjc==1) {
		...
		AddTimes(X_side,X,VL+((fjc+1)/2-1)*M,M);
	}
	...
	switch(gradients) {
		case 1:
			...
			if (fjc==1) {
				...
				for (j=0; j<FJC/2; j++) {	
					AddTimes(X_side+j,X,LAMBDA+j*M+j,M-j);
					AddTimes(X_side,X+j,LAMBDA+(FJC-j-1)*M,M-j);
				}
				AddTimes(X_side,X,LAMBDA+((FJC+1)/2-1)*M,M);
			}
			break;
		case 2:
			//set_bounds(X);
	...
	switch(gradients) {
		case 1:
			...
			if (fjc==1) {
				...
				if (geometry=="planar") {
					for (j=0; j<FJC/2; j++) {
						AddTimes(gs+j,gs_1,LAMBDA+j*M+j,M-j);
						AddTimes(gs,gs_1+j,LAMBDA+(FJC-j-1)*M,M-j);
					}
					AddTimes(gs,gs_1,LAMBDA+(FJC+1)/2*M,M);
				} else {cout << "non-planar not -yet- implemented" << endl; }
			}
			break;
		case 2:
			Zero(gs,M); set_bounds(gs_1);
	...
	switch(gradients) {
		case 1:
			if (fjc==1) {
				...
				for (j=0; j<fjc; j++) {
					X[j]=X[(BX1+1)*fjc-j-1];
					X[(MX+2)*fjc-j-1]=X[(BXM+1)*fjc-j-1]; 
				}				
			}
			break;
		case 2: 
		...
		case 1:
			...
			if (block) {
				if (fjc==1) {
					...
					for (x=r[0]*fjc; x<r[1]*fjc+1; x++)  H_MASK[x]=1;
				}
			} else {
