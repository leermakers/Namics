...
// Create lattice class instance and check inputs (reference above)
        ...
        if (!Lat[0]->CheckInput(start)) return 0;
...
// Error code for faulty variate class creation
        ...
        if (n_search > 0 && n_target == 0) {
            cout << "lonely search. Please specify in 'var' a target function, " <<
            " e.g. 'var : sys-NN : grand_potential : 0'" << endl;
            return 0;
        }
        if (n_target > 0 && n_search == 0) {
            cout << "lonely target. Please specify in 'var' a search function, " <<
            " e.g. 'var : mol-lipid : search : theta'" << endl;
            return 0;
        }
...
// last argument 1 is to read guess in X.
        ...
        TheEngine = SCF;
        if (!In[0]->MesodynList.empty()) { TheEngine = MESODYN; }
        if (!In[0]->ClengList.empty())   { TheEngine = CLENG; }
        if (!In[0]->TengList.empty())    { TheEngine = TENG; }
        ...
        switch (TheEngine) {
            case SCF:
                ...
                while (subloop < substart) {
                    ...
                    for (ii = 0; ii < n_out; ii++) Out[ii]->WriteOutput(subloop);
                    subloop++;
                }
                break;
        ...
        for (auto all_segments : Seg) all_segments->prepared = false;
        ...
        if (Sys[0]->initial_guess == "previous_result") {
            ...
            MONLIST.clear();
            STATELIST.clear();
            for (int i = 0; i < mon_length; i++)   MONLIST.push_back(Seg[Sys[0]->ItMonList[i]]->name);
            for (int i = 0; i < state_length; i++) STATELIST.push_back(Sta[Sys[0]->ItStateList[i]]->name);
        }
        ...
        if (Sys[0]->final_guess == "file") {
            ...
            for (int i = 0; i < mon_length; i++) MONLIST.push_back(Seg[Sys[0]->ItMonList[i]]->name);
            for (int i = 0; i < state_length; i++) STATELIST.push_back(Sta[Sys[0]->ItStateList[i]]->name);
            Lat[0]->StoreGuess(Sys[0]->guess_outputfile, New[0]->xx, New[0]->SCF_method, MONLIST, STATELIST,
                               Sys[0]->charged, start);
        }
