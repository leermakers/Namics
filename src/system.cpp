...
bool System::CheckInput(int start) {
  ...
  if (success) {
    ...
    if (cuda) {
      if (!GPU)
        cout << " program expect that you are going to use the GPU, but the input is not in line with this (either gradients < 3, or GPU != 'true' : compile without CUDA=1 flag." << endl;
      success = false;
    }
...
void System::PushOutput() {
  ...
  for (int i = 0; i < n_mon; i++)
    for (int j = 0; j < n_mon; j++)
      if (i != j)
        Seg[i]->push("chi-" + Seg[j]->name, CHI[i * n_mon + j]);
  string s = "profile;0";
  push("alpha", s);
  s = "profile;1";
...
bool System::ComputePhis() {
  ...
  if (charged) {
    Real phib = 0;
    for (int i = 0; i < n_mol; i++) {
      if (i != neutralizer && i != solvent)
        phib += Mol[i]->phibulk * Mol[i]->Charge();
    }
...
Real System::GetGrandPotential(void) {
  ...
  for (int i = 0; i < n_mol; i++) {
    ...
    int N = Mol[i]->chainlength;
    if (Mol[i]->IsTagged())
      N--; //One segment of the tagged molecule is tagged and then removed from GP through KSAM
    if (Mol[i]->IsClamped())
      N = N - 2;
    Cp(TEMP, phi, M);
